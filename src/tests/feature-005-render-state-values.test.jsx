...
const currentMenuItems = data.filter((item) => {
  return (
    item.food_category === burgersCategory &&
    item.restaurant === inNOutRestaurant
  );
});
const doubleDoubleMenuItemData = currentMenuItems.find(
  (item) => item.item_name === doubleDoubleMenuItem
);
...
export function testRenderStateValues(App) {
  ...
  FeatureTestSuite.before((ctx) => {
    ...
    const categoryChips = results.Chip.filter((chip) =>
      chip.parent.props.className.split(" ").includes("categories")
    );
  ...
  function setupMenuItems() {
    ...
    assert.not.ok(
      menuItemChip,
      `Menu item Chips should not be rendered before a category and restaurant are selected`
    );
    ...
    assert.ok(
      categoryChip,
      `Category Chip for ${burgersCategory} could not be found`
    );
    assert.ok(
      restaurantChip,
      `Restaurant Chip for ${inNOutRestaurant} could not be found`
    );
  ...
  async function getDoubleDoubleMenuItem(container) {
    ...
    const chipChildNodes = childNodes.filter(
  ...
  FeatureTestSuite.test(
    "Clicking a `category` Chip and a `restaurant` Chip renders `menuItem` Chips",
    async (ctx) => {
      ...
      assert.ok(
        doubleDoubleChip,
        `Menu Item Chips should be rendered after a category and restaurant are selected.`
      );
    }
  );

  FeatureTestSuite.test(
    "Clicking on a menu item renders the `NutritionalFacts` component",
    async (ctx) => {
      const renderQueries = setupMenuItems();
      const doubleDoubleChip = await getDoubleDoubleMenuItem(
        renderQueries.container
      );

      assert.ok(
        doubleDoubleChip,
        `Menu Item Chip for ${doubleDoubleMenuItem} could not be found`
      );

      fireEvent.click(doubleDoubleChip);

      // ensure the nutritional facts component is rendered
      const nutritionalFacts = renderQueries.queryByText("Nutrition Facts", {
        selector: "h3",
      });
      assert.ok(
        nutritionalFacts,
        `NutritionalFacts component should be rendered after clicking on a menu item`
      );

      // TODO: Determine if this is actually what we want to do here
      const { asFragment } = render(
        <NutritionalLabel item={doubleDoubleMenuItemData} />
      );
  ...
  FeatureTestSuite.test(
    ...
    async (ctx) => {
      const CustomNutritionLabelComponent = () => (
        <NutritionalLabel item={doubleDoubleMenuItemData} />
      );
      ...
      const NutritionalLabelFacts =
        NutritionLabelResults.results?.NutritionalLabelFact;

      assert.equal(
        NutritionalLabelFacts?.length,
        9,
        `There should be 9 NutritionalLabelFact components rendered`
      );
    }
  );
  ...
  FeatureTestSuite.test(
    ...
    async (ctx) => {
      ...
      const { container, queryByText } = render(
        <CustomNutritionLabelComponent />
      );

      const itemName = queryByText(doubleDoubleMenuItemData.item_name, {
        selector: "h4",
      });
      assert.ok(
        itemName,
        `NutritionalLabel component should render the correct item name inside the \`h4\` element`
      );
      ...
      for (const item of nutritionalFactDataWithCounts) {
        ...
        const factLabel = within(nutritionalLabelEl).queryByText(item.label, {
          selector: "span",
        });
        assert.ok(
          factLabel,
        ...
        if (item.value) {
          const factValues = within(nutritionalLabelEl).queryAllByText(
            item.value,
            { selector: "span.fact-value" }
          );
          assert.equal(
            factValues.length,
            item.count,
